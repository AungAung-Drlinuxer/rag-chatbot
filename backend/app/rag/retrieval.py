"""Hybrid RAG retrieval — vector + keyword (RRF) search + context assembly.

Design doc §3: ③ KB search (pgvector) → ⑤ context assembly. Embeddings/store
live in `app.rag.embeddings`; rerank in `app.rag.reranker`. Raw keyword SQL is
isolated in `app.persistence.search` (GOVERNANCE Rule 3).
"""
...
def _keyword_search(query: str, k: int, filter_clause: dict | None) -> list[dict]:
    ...
    Raw SQL is isolated in app.persistence.search (GOVERNANCE Rule 3); this
    module only formats rows. Domain filter applied client-side (small
    corpus). Falls back to ILIKE term matching when FTS finds nothing.
    """
    from app.persistence.search import fts_chunk_rows, ilike_chunk_rows
    ...
    results: list[dict] = []
    try:
        rows = fts_chunk_rows(terms, k)
        if not rows:
            rows = ilike_chunk_rows(terms, k)
...
def retrieve(query: str, domain: str | None = None, k: int | None = None) -> list[RetrievalHit]:
