...
backend/
├── app/
│   ├── main.py            # FastAPI app, routes, lifespan (init_db + telemetry), CORS
│   ├── config.py          # pydantic-settings (env-driven; H_CHAT_* via validation_alias)
│   ├── db.py              # SQLAlchemy engine/session, init_db (create_all + vector ext)
│   ├── models.py          # ORM: ChatSession, ChatMessage, Feedback, JiraTicket, KbMeta, EmailContact, AuditLog
│   ├── schemas.py         # pydantic request/response models
│   ├── llm.py             # stream_answer + LCEL chains (anthropic | openai) + fallback
│   ├── orchestration.py   # run_rag: classify→rewrite→retrieve→context→gate→llm
│   ├── classifier.py      # domain classifier (heuristic/embedding)
│   ├── rewriter.py        # query rewrite (follow-up/identity) — heuristic
│   ├── gate.py            # confidence gate (DECISION_ANSWER / DECISION_CAUTION)
│   ├── chunker.py         # chunk_text (tokens/overlap)
│   ├── rag.py             # PGVector store + retrieve + add_texts (get_vectorstore)
│   ├── ingest.py          # sync_all: load→chunk→embed→upsert (idempotent, change-detect)
│   ├── loaders.py         # multi-source: load_confluence + load_fileshare
│   ├── redis.py          # standalone | sentinel client (broker_url / get_redis)
│   ├── telemetry.py       # OTel→LGTM (best-effort no-op when endpoints empty)
│   ├── audit.py           # best-effort audit_log writes
│   ├── seed.py            # dev seed (contacts, KB docs)
│   ├── auth/              # ldap_auth | jwt | deps (get_current_user) | rbac (roles/ACL)
│   └── integrations/      # confluence | contacts | jira
├── workers/
...
├── tests/                 # pytest (41) — auth/rbac/jira/ingest/telemetry/audit/cors/llm/redis
├── kb_files/              # sample runbooks (file-share loader)
├── Dockerfile             # uv slim, non-root, CMD uvicorn
